				...
				<c:if test="${member.id eq board.memberId }">
					<span>
						<button type="button" class="btn btn-primary"
							onclick="move('/board_modify_form.do?no=${board.no }&title=${board.title}&content=${board.content}&memberId=${board.memberId}')">수정</button>
						<button type="button" class="btn btn-primary"
							onclick="move('/board_delete.do?no=${board.no }')">삭제</button>
					</span>
				</c:if>
				<span>
					<button type="button" style="float: right" class="btn btn-primary"
						onclick="move('/board_list.do')">이전</button>
				</span>
			</div>
		</div>
	</div>
